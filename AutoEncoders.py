...
class SAE(nn.Module):
    def __init__(self, ):
        ...
        self.fullConnection1 = nn.Linear(nb_movies, 20)
        self.fullConnection2 = nn.Linear(20, 10)
        self.fullConnection3 = nn.Linear(10, 20)
        self.fullConnection4 = nn.Linear(20, nb_movies)
        self.activationFunction = nn.Sigmoid()
    
    def forward(self, x):
        x = self.activationFunction(self.fullConnection1(x))
        x = self.activationFunction(self.fullConnection2(x))
        x = self.activationFunction(self.fullConnection3(x))
        x = self.fullConnection4(x)
        return x
    
   
sae = SAE() # Create the AutoEncoder
...
for epoch in range(1, nb_epoch + 1):
    train_loss = 0
    s = 0. # Number of users that rated at least one movie
    for id_user in range(nb_users):
        ...
        if torch.sum(target.data > 0) > 0:
            ...
            loss.backward() # Decides the direction to which the weights will be updated
